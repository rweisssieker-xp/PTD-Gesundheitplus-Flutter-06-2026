...
- The notification center shows system notification permission state, has widget coverage for sending blocked users to app settings, and surfaces per-notification states for active, permission-missing, system-blocked, needs-reschedule, and inactive reminders.
...
- Structured health passes are included in the offline emergency profile, visible offline emergency screen, QR payload, and local JSON export.
...
- The AI coach remains local by default, labels local-only answers, builds consent-gated bounded local context only after explicit consent, includes structured health passes in that bounded context, and stores a visible recovery answer when an optional configured online responder fails without changing health records.
