...
| Heilberufe | `/health/professionals` | Health record | implemented | User can search local specialty/provider suggestions, add a suggestion to local healthcare professionals, turn spoken-style German Behandler text into local professional drafts on-device, create, list, delete, and hear a native read-aloud summary. |
| Allergien | `/health/allergies` | Health record | implemented | User can create, edit, group, list, turn spoken-style German allergy text into local allergy drafts on-device, hear a native read-aloud summary, run a local medication-allergy conflict check, and delete local allergies with severity warnings. |
| Termine | `/appointments` | Appointments | implemented | User can create, list, complete, delete, hear a native read-aloud summary, schedule native local reminders, turn spoken-style German text into local appointment drafts on-device, export appointments as local `.ics` calendar files, and see a recovery message if file creation or sharing fails. |
...
| Medikamenten-Tagesplan | `/medication/daily-plan` | Medication management | implemented | User can generate daily medication logs, mark intake as taken or skipped, and confirm intake from spoken-style German text with local `confirmedByVoice` tracking. |
...
| Notfallprofil | `/emergency/profile` | Emergency | implemented | User sees a QR-backed emergency profile built from local records and can use native phone, SMS, WhatsApp, system share, and current-location SMS handoff for emergency contacts with visible failure states. |
...
| Offline-Notfall | `/emergency/offline` | Emergency | implemented | Emergency screen renders local medication, allergy, contact, QR payload, and native phone/SMS/WhatsApp/share/current-location handoff without backend access. |
| Dokumenten-Scan | `/documents/scan` | Documents and scanning | implemented | User can capture or attach document images after native permission checks, recover from blocked camera/gallery permissions through app settings, preview local medical insights, and store metadata locally. |
...
| Daten-Export | `/export` | Local export and sharing | implemented | User can export local health record JSON, share the generated file, and see recovery messages if local file creation or sharing fails. |
